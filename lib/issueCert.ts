...
export async function issueCert(input: {
  ...
  // Render PDF — DB-backed data-driven path with legacy fallback (shared
  // helper; same path used by sendApprovedCert/certPipeline/voidCert).
  let pdfBytes: Uint8Array;
  try {
    pdfBytes = await renderCertificateWithFallback(admin, formId, coiInput, { certNumber });
  } catch (err) {
